<template>
	<view>
		...
		<view class="goods-list">
			...
			<view class="loading-text" v-if="productList.length > 0 ">{{ loadingText }}</view>
		</view>
		<view class="publish">
			<image src="../../../static/main/plus.png" mode="" @tap="goPublish"></image>
		</view>
	</view>
</template>
...
export default {
	...
	onLoad() {
		window.onload = '/pages/tabBar/home/home'
		// #ifdef APP-PLUS
	...
	methods: {
		...
		initWaterFall(){
			this.$nextTick(()=>{
				...
				if($goodsItem.$children) {
					let leftHeightTotal = 0 
					let rightHeightTotal = 0 ;
					$goodsItem.$children.forEach((c,i)=>{
						let goodsItem = {} 
						let elHeight = c.clientHeight + this.itemMarginBottomSize
						...
						this.goodsItemStyle.push(goodsItem)
					})
					
					this.boxHeight =  leftHeightTotal < rightHeightTotal ? rightHeightTotal : leftHeightTotal + 'px'
				}
			})
		...
		getAllProduction(){
			...
			uni.request({
				url : interfaces.getAllProduction,
				method: "GET",
				header: {
		...
		toCategory(e) {
			uni.navigateTo({
				url: `../../goods/goods-list/good-list?cid=${e.id}&name=${e.name}`
			});
		},
		//推荐商品跳转
		toPromotion(e) {
