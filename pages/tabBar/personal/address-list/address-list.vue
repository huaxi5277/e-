<template>
	<view class="container">
		<view class="address-list" v-if="addressList.length > 0 ">
			<view class="a-address" v-for="(item,index) in addressList" :key="index" @tap="select(item)">
				...
				<view class="right-edit" @tap.stop="editAddess(item.llid)"></view>
			</view>
		</view>
		<view class="no-addr" v-else style="background-color: #fff;text-align: center;margin-top: 20px;">
			暂无地址
		</view>
...
<script>
	...
	export default {
		...
		methods: {
					...
					success : (ret)=>{
						this.request({
							...
							success :(ret)=>{
								let list = [] 
								let temp = {}
								ret.addr.forEach((c,i)=>{
									temp = {
												id: i+1,
												linkMan: c.name,
												mobile: c.phone,
												address: `${c.region}-${c.detailed}`,
			...
			editAddess(id) {
				uni.navigateTo({
					url: `../../../city/city?id=${id}&type=edit`
				})
			},
			addAddess() {
				uni.navigateTo({
					url: "../../../city/city?type=add"
				})
			},
			select(row){
				...
				if(this.type) {
				   uni.navigateTo({
				   	url: "../../../order/order?row="+ JSON.stringify(row) + "&type=backSelect" + "&params=" +  this.params
				   })	
				}
			}
		}
