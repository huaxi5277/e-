				...
				//实际应用中请提交ajax,模板定时器模拟提交效果
		           if(this.editType == 'edit') {
					   uni.getStorage({
					   	...
					   	success : (ret)=>{
					   		this.request({
					   			url : interfaces.userUpdateAddress,
					   			method: "POST",
					   			header: {
					   				'Authorization': 'Bearer ' + JSON.parse(ret.data).token
					   			},
		...
		onLoad(e) {
			...
			if(e.type=='edit'){
				uni.getStorage({
					...
					success : (ret)=>{
						this.request({
							url : interfaces.getUserAddressThoughtId,
							method: "POST",
							header: {
								'Authorization': 'Bearer ' + JSON.parse(ret.data).token
							},
							...
							success :(ret)=>{
								this.name = ret.addr.name
								this.tel = ret.addr.phone
								this.detailed = ret.addr.detailed
								this.isDefault = ret.addr.isDefault
								this.region.label = ret.addr.region
								this.id =  ret.addr.id
								this.llid = ret.addr.llid
							},
							fail : (err)=>{
								uni.navigateTo({
									url: '../../login/login'
