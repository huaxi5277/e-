...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.result = JSON.parse(e.result)
			...
			doDeposit(){
				...
				setTimeout(()=>{
					...
					uni.getStorage({
						...
						success : (ret)=>{
							this.request({
								url : interfaces.userUpdateOrderStatus,
								method: "POST",
								header: {
								...
								success :(ret)=>{
									if(ret.msg){
										if(ret) {
										  ...
										  setTimeout(()=>{
										  	uni.redirectTo({
										  		url: "../tabBar/personal/pay-success/pay-success?amount=" + this.amount
										  	});
										  },300);	
										}
									} else {
